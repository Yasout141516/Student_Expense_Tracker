...
const Expenses = () => {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      // Calculate stats
      const total = response.data.data.reduce((sum, exp) => sum + exp.amount, 0);
...
const handleCreateCategory = async (values) => {
  try {
    ...
    // ⭐ KEY FIX: Use setTimeout to ensure state updates before setting form value
    setTimeout(() => {
      if (newCategoryId) {
        form.setFieldsValue({
        ...
        const newCategory = expenseCategories.find(cat => cat.type === values.type);
        if (newCategory) {
          form.setFieldsValue({
